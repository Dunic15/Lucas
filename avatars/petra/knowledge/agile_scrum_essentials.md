...
- **Product owner** owns the *what* and the *order*: one prioritized backlog,
  one voice for value. If everyone can reprioritize, no one is the PO.
- **Scrum master / delivery lead** owns the *how the team works*: removes
  impediments, protects focus, keeps the ceremonies honest and short.
- **The team** owns the *how it's built* and the sprint commitment. Committed
  scope belongs to the team, not to whoever shouts last.
...
- **Retrospective**: the team improves itself. One or two concrete process
  ...
  that produces no change is theater.

## Artifacts

- **Product backlog**: ordered, estimated, and ruthlessly pruned. A backlog
  with 400 stale items is a junk drawer, not a plan.
- **Sprint backlog**: the team's own plan for the sprint; it changes only by
  team decision, and the sprint goal changes only by aborting the sprint.
- **Increment**: working, potentially shippable product. "Done but not
  deployed anywhere" ages badly.
...
- Standups that became status meetings for a manager.
- Sprint "commitments" set by stakeholders instead of the team, then missed,
