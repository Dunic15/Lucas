...
- **Access level**: hiring manager approval (mandatory before IT provisions).
- **Elevated/admin access**: additional approval from the Security lead.

## Owners

- People Ops owns the overall checklist and policy acknowledgment.
- IT owns access provisioning.
- The hiring manager owns start-date confirmation and access-level approval.

## Definition of done

Onboarding is complete only when steps 1–7 are all recorded, the signed policy
acknowledgment is in the HRIS, and an onboarding buddy is assigned.

## Common gaps (what teams forget)

- Starting provisioning before the manager has approved the access level.
- No named owner for the access approval.
- Training date never put on the calendar.
- Policy acknowledgment collected verbally but never signed.
