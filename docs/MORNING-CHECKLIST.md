...
| **Execution Pack** | #62 ✅ | Avatar **itself** emails the recap + files a Drive notes doc at meeting end | **OFF** (`EXECUTE_ENABLED`) | ✅ scopes gmail.send+drive.file live |
...
### A. Turn on autonomous execution (recap email + Drive notes)
1. **Reconnect Google once** for the send/write scopes:
   `https://dhfgfe6yw6.eu-central-1.awsapprunner.com/oauth/google/connect`
   logged in as **laura.ai.122222@gmail.com** → approve (Advanced → continue).
2. **Tell me to flip it** (I'll set on App Runner in 1 min): `EXECUTE_ENABLED=true`
   + `EXECUTE_RECAP_TO=<your email>`. Left OFF deliberately — it sends real
   email, so it's your call. **Recap goes ONLY to `EXECUTE_RECAP_TO`** (never
   silently to meeting attendees — safety fix from review). Notes → Cedric's
   Drive folder.

### B. Cedric's Slack side (Cedric#4) — for Ben
...
### C. Optional — prettier email address
...
7. End: *"Cedric, you can leave."* Then send me the meeting link — I'll pull the
   ...
   meter stopped) and, if execution is on, confirm the recap email + Drive note.
...
- **Recap email arriving** — needs step 2A + the flag; wiring is tested (250 tests).
- **Ben's Slack execution** — his PR awaits review.
- **Calendar-event auto-booking** — `create_calendar_event` is written but not
  wired (roadmap NOW #1); dropped its scope until wired.
