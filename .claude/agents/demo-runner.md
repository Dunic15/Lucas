...
4. Smoke-test:
   - `POST /demo/ask` with `{"question":"What approvals are needed before provisioning?","avatar_id":"sofia"}` → expect a non-empty `answer` and `citations`.
   - `POST /demo/post_meeting` with the sample transcript from `GET /demo/sample?avatar_id=sofia` → expect `summary`, `checklist`, `follow_up_email`.
