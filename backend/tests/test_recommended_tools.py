...
avatar.yaml declares `recommended_tools`; the avatar loader normalizes them;
/dashboard/summary enriches each entry with org connect state + the connect
door the frontend renders. Key-free: no vendors are called — connect state
comes from the (empty) tmp sqlite store and a disabled Pipedream.
"""
...
def test_summary_carries_cedric_tools_with_doors(client):
    ...
    # builtin + soon (contacts is roadmap on this line) → connected, no door
    contacts = tools["contacts"]
    assert contacts["soon"] is True
    assert contacts["connected"] is True
    assert contacts["connect"] is None
    # pipedream (disabled key-free) → not connected, plain GET connect door
    cly = tools["calendly"]
    assert cly["connected"] is False
    assert cly["connect"] == {
        "type": "href", "url": "/dashboard/pipedream/connect?app=calendly"
    }
