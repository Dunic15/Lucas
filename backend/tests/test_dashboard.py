...
def test_summary_shape_and_attribution(client):
    _seed_artifact(avatar_id="cedric")
    ...
    # The rollup attributes the meeting to cedric, not the other avatars.
    cedric = next(a for a in data["avatars"] if a["id"] == "cedric")
    others = [a for a in data["avatars"] if a["id"] != "cedric"]
    assert cedric["meetings_total"] == 1
    assert all(a["meetings_total"] == 0 for a in others)
