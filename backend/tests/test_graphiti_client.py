...
# ─────────────── local-stack wiring (Anthropic + fastembed) ───────────────
...
def test_construct_wires_laura_stack_no_openai(monkeypatch):
    ...
    LOCAL cosine reranker — no OpenAI, no new key. Skipped cleanly when
    graphiti-core isn't installed (the feature stays optional)."""
    ...
    # Anthropic LLM (never graphiti-core's default OpenAI client).
