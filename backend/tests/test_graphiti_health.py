...
Anthropic extraction) end-to-end without a live meeting: GET ?run=1 runs a real
ingest→recall against a dedicated __smoke__ group. Here we cover the cheap paths
(status shape, disabled-guard, auth gate) — the live round-trip needs a real
graph DB, so it is exercised on the deploy, not in the key-free suite.
"""
