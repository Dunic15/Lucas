...
| File | Contents |
|---|---|
| `callback.py` | Signed callbacks to the orchestrator: `send_status`, `send_ended` (retried), `fetch_context`. HMAC (`X-Laura-Signature`) + Bearer, follows one redirect hop. |
| `integration.py` | Logic lifted out of `main.py`: `auth_error` (Bearer gate), `MeetingContext`, `MAX_BRIEF_BYTES`/`brief_too_large`, `build_integration`, `deliver_ended`, `notify_failed`, `handle_webhook_status`, `inject_brief`. |
...
callback signing/retries, cancel, integration wiring). Run the whole suite with
