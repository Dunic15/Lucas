...
"""Meeting-lifecycle core — the bot dispatch (_start_avatar_session) and the
post-meeting finalize (_finalize_session/_finalize_session_locked) plus their
meter/usage/artifact/recall helper closure. Hoisted out of main.py so the api/
route groups (sessions, health, console) can share it without importing main
(which would be circular). main.py re-imports the names its stayers still call.
"""
from typing import Optional
import asyncio, time, uuid, traceback, re, httpx
from fastapi.concurrency import run_in_threadpool
from ..config import settings
from . import meeting_state
from .. import store
from .. import recall_client
from .. import anam_client
