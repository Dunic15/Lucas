...
def adaptive_deference_seconds(
    ...
    *,
    ...
    When enabled, the wait adapts to signals already on hand:
      • a human partial is mid-utterance (``since_partial`` small) → wait ``hi``
        (give the human more room to take the floor);
      • only one human present → wait ``lo`` (respond snappily, no one to defer to);
    ...
    if not enabled or not (0 < lo < hi):
        return base
    if since_partial < active_partial_seconds:
        wait = hi
    elif n_humans <= 1:
        wait = lo
    elif is_question:
